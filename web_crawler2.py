# -*- coding: utf-8 -*-
"""
Created on Fri Mar 16 01:19:00 2018
...
import time
import os
os.chdir('C:\\Users\\fhchanghao\\Desktop\\yy\\3.13\\')
data=pd.read_excel(u'电影名字.xlsx')
moive=list(set(data[u'影片名']))
moive1=list(set(data[u'movie']))
def getProxyIp():
    proxy = []
    for i in range(1,3):
...
#     print url
    ...
    sid=re.findall("'dou_search_movie', sid: [0-9]{7,8}",res)[0]
...
comment=[]   #50,51
yp1=[]
sy=list(set(idlist).difference(set(idlist1)))
sy=['26387939']
for j in sy:
    yp1.append(j)
    #len1 = len(proxies)-1
    for p in range(0,600,20):
        proxie = {
            "http":"1.2.3.4:2222"
        }
        ...
        url = 'https://movie.douban.com/subject/'+str(j)+'/'+'comments?start=' + str(p) + '&limit=20&sort=new_score&status=P&percent_type='
        print(url)  #proxies=proxie
        html =requests.get(url,headers=header,timeout=100).text
        print(html)
        dbdata=pcinfo(html)
        comment.append(dbdata)
        time.sleep(10)
d1=pd.DataFrame(pd.concat(comment))
d2=d1.dropna()
d2=[]
for i in moive:
    d2.append(data[data.iloc[:,0]==i].iloc[0:200,:])
d3=pd.concat(d2)
...
def pcinfo1(html):
    ...
    movie=re.findall(r'[\u4e00-\u9fa5]{2,10}',str(title))
    olArticle = soup.find_all('div', class_='abstract')
    dy=re.findall(u'导演:(.*?)\\n',str(olArticle))
    #year=re.findall(u'年份:(.*?)\\n',str(olArticle))
    type1=re.findall(u'类型:(.*?)\\n',str(olArticle))
    com= soup.find_all('blockquote', class_='comment')
    pf=[]
    fxlb=[]
    date=[]
    for i in range(len(com)):
        print(i)
        pf.append(re.findall(u'总票房：(.*?)万元',str(com[i])))
        fxlb.append(re.findall(u'发行类别：(.*?)\\n',str(com[i]))[0])
        date.append(re.findall(u'上映日期：(.*?) |$',str(com[i]))[0])
    zd=[movie,type1,pf,fxlb,date,dy]
    result=pd.DataFrame(zd)
    result=result.T
    return(result)
d3212=[]
for p in range(0,3400,25):
        #proxie = {
            #"http":"1.2.3.4:2222"
        #}
        #index = random.randint(0, len1)
        #proxie["http"] = proxies[index]
        url = 'https://www.douban.com/doulist/1295618/?start=' + str(p) + '&sort=seq&sub_type='
        print(url)  #proxies=proxie
        html =requests.get(url,headers=header,timeout=100).text
        print(html)
        dbdata=pcinfo1(html)
        d3212.append(dbdata)
        time.sleep(10)
d36=pd.DataFrame(pd.concat(d3212))
###########################################情感得分析
